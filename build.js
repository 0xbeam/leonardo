...
/**
 * Leonardo Design System — Build & Push
 *
 * Usage:
...
const MARKER_START = '/* LEONARDO-START */';
const MARKER_END = '/* LEONARDO-END */';
// Backward compat: also detect old markers in apps that haven't been updated yet
const LEGACY_MARKER_START = '/* GRAVITY-START */';
const LEGACY_MARKER_END = '/* GRAVITY-END */';
...
function pushToApp(appName, appConfig) {
  ...
  const leonardoCSS = buildCSS(appConfig.theme, appConfig.extensions || []);

  // Check for existing markers (new or legacy)
  const hasNewMarkers = original.includes(MARKER_START) && original.includes(MARKER_END);
  const hasLegacyMarkers = original.includes(LEGACY_MARKER_START) && original.includes(LEGACY_MARKER_END);
  const hasMarkers = hasNewMarkers || hasLegacyMarkers;

  // Use whichever markers exist in the file for finding boundaries
  const startMarker = hasNewMarkers ? MARKER_START : LEGACY_MARKER_START;
  const endMarker = hasNewMarkers ? MARKER_END : LEGACY_MARKER_END;

  let newContent;
  if (hasMarkers) {
    // Replace between markers, always write new markers
    const before = original.substring(0, original.indexOf(startMarker));
    const after = original.substring(original.indexOf(endMarker) + endMarker.length);
    newContent = before + MARKER_START + '\n' + leonardoCSS + '\n' + MARKER_END + after;
  } else {
    // First time: wrap entire file content
...
function buildDist() {
  ...
  fs.writeFileSync(path.join(DIST_DIR, 'leonardo.css'), full, 'utf-8');

  // TW4 variant
  fs.writeFileSync(path.join(DIST_DIR, 'leonardo-tw4.css'), full, 'utf-8');

  // TW4 inline (Next.js)
  const inline = buildCSS('@theme inline');
  fs.writeFileSync(path.join(DIST_DIR, 'leonardo-tw4-inline.css'), inline, 'utf-8');

  // Workbench
  fs.writeFileSync(path.join(DIST_DIR, 'index.html'), buildIndex(), 'utf-8');

  console.log('Built dist/:');
  console.log('  leonardo.css');
  console.log('  leonardo-tw4.css');
  console.log('  leonardo-tw4-inline.css');
